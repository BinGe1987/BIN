...
public class FacilityData extends HTData {

    public List<Facility> list = new ArrayList<>();

    @Override
    public void onData() {
        list.clear();
        if (isDataNormal()) {
            JSONArray array = JSONArrayByKey("list_facility");
    ...
    public Facility getItem(int index) {
        if (index < list.size()) {
